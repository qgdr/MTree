...
distrcerr = OffsetArray(zeros(J-1, N), 1:J-1, 0:N-1)
for n in 0:N-1
    for j in 1:s+1
        distrcerr[j, n] = -(U[j, n+1] - U[j, n])/τ + ( a_mhh[j+1]*(U[j+1, n+1] - U[j, n+1])/h - a_mhh[j]*(U[j, n+1] - U[j-1, n+1])/h ) / h + F[j, n+1]
    end
    for j in s+2:J-1
        distrcerr[j, n] = -(U[j, n+1] - U[j, n])/τ + ( a_mhh[j+1]*(U[j+1, n] - U[j, n])/h - a_mhh[j]*(U[j, n] - U[j-1, n])/h )/ h + F[j, n]
    end
end
@show distrcerr .|> abs |> maximum



println("Error:")
@show U_exact - U .|> abs |> maximum;
